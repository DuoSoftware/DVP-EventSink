...
if (config.evtConsumeType === "amqp") {
  ...
  connection.on("ready", function() {
    ...
    logger.debug(
      "[DVP-EventService.AMQPConnection] - [%s] - AMQP Connection READY"
    );
    readyCount += 1;
    if (readyCount === 1)
      connection.queue(
        ...
        function(q) {
          ...
          q.subscribe(function(message) {
            ...
            console.log(
              "TriggerToIntegration : " + config.triggerToIntegrations
            );
            ...
            if (
              (evtClass === "CALL" ||
                ...
                evtClass === "AGENT" ||
                evtName === "ards-added") &&
              config.triggerToIntegrations === "true" &&
              evtSpecificData
            ) {
              var evtype = "";
              ...
              //Call API
              externalApiHandler.EventTrigger(
                companyId,
                tenantId,
                evtype,
                evtSpecificData
  ...
  redisHandler.redisClient.on("message", function(channel, message) {
    if (channel && channel === "SYS:MONITORING:DVPEVENTS") {
      ...
      logger.debug(
        "[DVP-EventService.DVPEVENTS] - Event Received - Params - message : %s, appId : %s",
        message
      );
