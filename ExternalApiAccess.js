...
var EventTrigger = function (companyId, tenantId, evtType, payload) {
  try {
    ...
    var httpUrl = util.format(
      "http://%s/DVP/API/1.0.0.0/EventTrigger/Trigger?eventType=%s",
      eventTriggerIp,
      evtType
    );

    if (config.dynamicPort || validator.isIP(eventTriggerIp)) {
      httpUrl = util.format(
        "http://%s:%d/DVP/API/1.0.0.0/EventTrigger/Trigger",
        eventTriggerIp,
        eventTriggerPort,
        evtType
      );
    ...
    logger.debug(
      "[DVP-EventSink.EventTrigger] - Creating Api Url : %s",
      httpUrl
    );
    ...
    httpReq.post(options, function (error, response, body) {
      if (!error && response.statusCode >= 200 && response.statusCode <= 299) {
        logger.debug(
          "[DVP-EventSink.EventTrigger] - Call Event Trigger Success : %s",
          body
        );
      } else {
        logger.error(
          "[DVP-EventSink.EventTrigger] - Call Event Trigger Fail",
          error
        );
      }
    });
