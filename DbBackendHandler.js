  ...
  try {
    dbModel.DVPEvent.findAll({
      ...
      .then(function (evtList) {
        logger.debug(
          "[DVP-EventService.GetEventDataBySessionId] PGSQL Get dvp event records for sessionId query success"
        );
      ...
      .catch(function (err) {
        logger.error(
          "[DVP-EventService.GetEventDataBySessionId] PGSQL Get dvp event records for sessionId query failed",
          err
        );
...
var GetDevEventDataByAppIdAndDateRange = function (
  type,
  appId,
  starttime,
  endtime,
  ...
  try {
    ...
    dbModel.DVPEvent.findAll({
      attributes: [
        "EventParams",
        [
          dbModel.SequelizeConn.fn(
            "count",
            dbModel.SequelizeConn.col("SessionId")
          ),
          "count",
        ],
      ],
      where: query,
      group: ["CSDB_DVPEvent.EventParams"],
    })
      .then(function (evtList) {
        logger.debug(
          "[DVP-EventService.GetDevEventDataBySessionId] PGSQL Get dvp event records for appid and type success"
        );
        callback(undefined, evtList);
      })
      .catch(function (err) {
        logger.error(
          "[DVP-EventService.GetDevEventDataBySessionId] PGSQL Get dvp event records for appid and type query failed",
          err
        );
        callback(err, emptyList);
      });
...
var GetAllEventsByNodes = function (
  startDate,
  endDate,
  type,
  appId,
...
var GetAllEventsByNodesCount = function (
  startDate,
  endDate,
  type,
  appId,
...
var GetDevEventDataBySessionId = function (sessionId, callback) {
  ...
  try {
    dbModel.DVPEvent.findAll({
      where: [{ SessionId: sessionId }, { EventCategory: "DEVELOPER" }],
      order: ["EventTime"],
    })
      .then(function (evtList) {
        logger.debug(
          "[DVP-EventService.GetDevEventDataBySessionId] PGSQL Get dvp event records for sessionId and cat query success"
        );
      ...
      .catch(function (err) {
        logger.error(
          "[DVP-EventService.GetDevEventDataBySessionId] PGSQL Get dvp event records for sessionId and cat query failed",
          err
        );
        callback(err, emptyList);
      });
...
var GetEventDataByClassTypeCat = function (
  evtClass,
  evtType,
  evtCategory,
  callback
  ...
  try {
    dbModel.DVPEvent.findAll({
      ...
      .then(function (evtList) {
        logger.debug(
          "[DVP-EventService.GetEventDataByClassTypeCat] PGSQL Get dvp event records for ClassTypeCat query success"
        );
      ...
      .catch(function (err) {
        logger.error(
          "[DVP-EventService.GetEventDataByClassTypeCat] PGSQL Get dvp event records for ClassTypeCat query failed",
          err
        );
        callback(err, emptyList);
      });
...
var GetCallCDRForAppAndSessionId = function (appId, sessionId, callback) {
  try {
    dbModel.CallCDR.find({ where: [{ Uuid: sessionId }, { AppId: appId }] })
      .then(function (cdr) {
        logger.debug(
          "[DVP-EventService.GetCallCDRForAppAndSessionId] PGSQL Get dvp event records query success"
        );
        callback(undefined, cdr);
      })
      .catch(function (err) {
        logger.error(
          "[DVP-EventService.GetCallCDRForAppAndSessionId] PGSQL Get dvp event records query failed",
          err
        );
...
var AddEventData = function (eventInfo, callback) {
  try {
    eventInfo
      ...
      .then(function (rsp) {
        logger.debug(
          "[DVP-EventService.AddEventData] PGSQL save dvp event record query success"
        );
        callback(undefined, true);
      })
      .catch(function (err) {
        logger.error(
          "[DVP-EventService.AddEventData] PGSQL save dvp event record query failed",
          err
        );
        callback(err, false);
      });
